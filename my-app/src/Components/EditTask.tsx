...
function EditTask({t,dateset,changeToday,day,onChange,onClickDay,edit,setEdit,editid,uploadData}:EditTask) {
    ...
    async function getEditData(){
        try{
          await axios.get(`http://localhost:3005/${editid}`)
          .then(res=>{
    ...
    return (
        <div className="AddPopup">
            <div className="inputadd">
                <input type="text" placeholder="Task name" className="taskname" value={value.taskName} onChange={e=>setValue({...value,taskName:e.target.value})}/>
                <input type="text" placeholder="Description" className="description" value={value.description} onChange={e=>setValue({...value,description:e.target.value})} />
            </div>
